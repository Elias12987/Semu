...
def approve_order(oid, volume_gb=0, expire_at=None):
    ...
    cur.execute("SELECT user_id, amount, volume_gb, plan FROM orders WHERE id=?", (oid,))
    row = cur.fetchone()
    if row:
        uid = row[0]
        vol = row[2] if row[2] and row[2] > 0 else 0
        plan = row[3] if row[3] else ""
        # فقط برای سفارشات واقعی (نه topup) امتیاز بده
        if plan != "topup":
...
async def admin_private_msg_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    if not is_admin(query.from_user.id): return ConversationHandler.END
    await query.edit_message_text("💬 پیام شخصی - /cancel برای انصراف")
...
async def admin_wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(query.from_user.id):
        return ConversationHandler.END
    await query.edit_message_text("💰 تغییر موجودی - /cancel برای انصراف")
...
async def admin_points_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(query.from_user.id):
        return ConversationHandler.END
    await query.edit_message_text("⭐ تغییر امتیاز - /cancel برای انصراف")
...
async def admin_topup_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # split: ['', 'topup', 'approve', 'OID', 'UID', 'AMOUNT']
