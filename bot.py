...
def add_points(uid, count=1):
    con = sqlite3.connect("vpn_bot.db")
    cur = con.cursor()
    cur.execute("UPDATE users SET points = points + ? WHERE user_id=?", (count, uid))
    con.commit()
    con.close()
...
def check_discount_code(uid, code):
    ...
    if code in DISCOUNT_CODES:
        ...
        if dc["first_only"] and get_order_count(uid) > 0:
...
def get_expiring_orders():
    con = sqlite3.connect("vpn_bot.db")
    cur = con.cursor()
    cur.execute("""SELECT o.id, o.user_id, o.expire_at, n.notif_5d, n.notif_3d, n.notif_1d, n.notif_1h, n.notif_exp
...
def main_menu():
    return ReplyKeyboardMarkup([
        ...
        [KeyboardButton("📋 اشتراک های من"), KeyboardButton("🎫 تیکت پشتیبانی")],
...
async def keyboard_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🛒 خرید VPN":
        ...
        can2 = points >= 5
        kb = []
        kb.append([InlineKeyboardButton("🎰 گردونه 1 - رایگان روزانه" + (" ✅" if can1 else " ⏳فردا"), callback_data="spin_wheel1" if can1 else "wheel_locked")])
        kb.append([InlineKeyboardButton("🎰 گردونه 2 - 5 امتیاز" + (" ✅" if can2 else " 🔒" + str(points) + "/5"), callback_data="spin_wheel2" if can2 else "wheel_locked")])
        kb.append([InlineKeyboardButton("🔙 برگشت", callback_data="back_main")])
        await update.message.reply_text(
            "🎰 گردونه شانس\n\n⭐ امتیاز شما: " + str(points) + "\n\n"
            "🎰 گردونه 1: هر روز یک بار رایگان\n"
            "🎰 گردونه 2: با 5 امتیاز\n\n"
            "💡 هر خرید = 1 امتیاز",
        ...
        kb = []
        if points >= 10:
            kb.append([InlineKeyboardButton("🎲 بینداز تاس! (10 امتیاز)", callback_data="roll_dice")])
        else:
            kb.append([InlineKeyboardButton("🔒 تاس نیاز به 10 امتیاز داره (" + str(points) + "/10)", callback_data="wheel_locked")])
        kb.append([InlineKeyboardButton("🔙 برگشت", callback_data="back_main")])
        await update.message.reply_text(
            "🎲 تاس شانس\n\n⭐ امتیاز شما: " + str(points) + "\n\n"
            "🎲 با انداختن تاس:\n"
            "- عدد تاس = گیگابایت کانفیگ\n"
            "- مثلاً 4 اومد = کانفیگ 4GB\n"
            "- مدت: 2 روزه\n"
            "- هزینه: 10 امتیاز",
            reply_markup=InlineKeyboardMarkup(kb))
            ...
            if expire_at:
                try:
                    ...
                    if diff.total_seconds() > 0:
                        msg += "⏰ انقضا: " + expire_at[:10] + " (" + str(diff.days) + " روز مانده)\n"
                    else:
                        msg += "⏰ منقضی شده!\n"
                except:
...
async def custom_plan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "حداقل: " + str(min_gb) + "GB\n"
        "قیمت: " + fmt(price_per_gb) + " تومان به ازای هر گیگ\n\n"
        "مقدار گیگابایت مورد نظرت رو بنویس (عدد):")
...
async def pay_wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if discount_code:
        use_discount_code(uid, discount_code)
    ...
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(admin_id, "✅ خرید از کیف پول\nکاربر: " + str(uid) + "\nپلن: " + plan_key + "\nمبلغ: " + fmt(price) + " تومان\nسفارش #" + str(oid))
        except:
            pass
    await query.edit_message_text(
...
async def roll_dice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if get_points(uid) < 10:
        await query.answer("امتیاز کافی نداری!", show_alert=True)
        return
    deduct_points(uid, 10)
    dice_emojis = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣"]
    result = random.randint(1, 6)
    msg = ("🎲 تاس انداخته شد!\n\n"
           "نتیجه: " + dice_emojis[result-1] + " = " + str(result) + " گیگابایت\n\n"
           "📩 کانفیگ " + str(result) + "GB به مدت 2 روز به زودی ارسال میشه!")
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(admin_id, "🎲 تاس شانس\nکاربر: " + str(uid) + "\nنتیجه: " + str(result) + "GB - 2 روزه")
        except:
            pass
    await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data="back_main")]]))
...
def main():
    ...
    KEYBOARD_BUTTONS = "^(🛒 خرید VPN|🎁 تست رایگان|💰 افزایش موجودی|🎰 گردونه شانس|🎲 تاس شانس|👥 دعوت از دوستان|👤 حساب من|📋 اشتراک های من|🎫 تیکت پشتیبانی|📞 پشتیبانی)$"
