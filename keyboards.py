...
def categories_keyboard(categories: list) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        icon = icons.get(cat, "📦")
        rows.append([InlineKeyboardButton(text=f"{icon} {cat}", callback_data=f"category:{cat}")])
...
def products_keyboard(products) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="🔙 بازگشت به دسته‌بندی‌ها", callback_data="back_to_categories")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
