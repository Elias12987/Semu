...
def init_db():
    with closing(get_conn()) as conn, conn:
        ...
        try:
            conn.execute("ALTER TABLE products ADD COLUMN category TEXT NOT NULL DEFAULT 'عادی'")
        except Exception:
            pass
        conn.execute(
            """
...
def add_product(name, category, price, duration_days, traffic_gb, inbound_id, description=""):
    with closing(get_conn()) as conn, conn:
        cur = conn.execute(
            "INSERT INTO products (name, category, price, duration_days, traffic_gb, inbound_id, description, active) "
            "VALUES (?,?,?,?,?,?,?,1)",
