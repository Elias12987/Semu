...
def update_points(uid, amount):
    ...
    if amount < 0:
        # امتیاز منفی نشه
        cur.execute("UPDATE users SET points = MAX(0, points + ?) WHERE user_id=?", (amount, uid))
    else:
...
def approve_order(oid, volume_gb=0, expire_at=None):
    ...
    row = cur.fetchone()
    if row:
        cur.execute("UPDATE users SET order_count = order_count + 1, total_gb = total_gb + ? WHERE user_id=?",
...
def update_wheel1_time(uid):
    ...
    cur = con.cursor()
    today = datetime.now().strftime("%Y-%m-%d")
    cur.execute("UPDATE users SET last_wheel1=? WHERE user_id=?", (today, uid))
    con.commit()
    con.close()
...
async def custom_plan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    panel_key = query.data[len("custom_plan_"):]
    panel = PANELS[panel_key]
    context.user_data["selected_panel"] = panel_key
    await query.edit_message_text(
...
async def pay_wallet_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    update_points(uid, 1)  # امتیاز خرید
...
async def spin_wheel2_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    uid = query.from_user.id
    # atomic: کم کن فقط اگه امتیاز کافی داره
    con = sqlite3.connect("vpn_bot.db")
    cur = con.cursor()
    cur.execute("UPDATE users SET points = points - 3 WHERE user_id=? AND points >= 3", (uid,))
...
async def roll_dice_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    con = sqlite3.connect("vpn_bot.db")
    cur = con.cursor()
    cur.execute("UPDATE users SET points = points - 5 WHERE user_id=? AND points >= 5", (uid,))
    changed = cur.rowcount
    con.commit()
    con.close()
    if not changed:
...
async def admin_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(query.from_user.id, "متن پیام رو بفرست - برای انصراف /cancel بزن")
...
async def admin_private_msg_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(query.from_user.id, "آیدی عددی کاربر رو بفرست - برای انصراف /cancel بزن")
...
async def receive_admin_wallet_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not uid:
            await update.message.reply_text("❌ آیدی کاربر پیدا نشد! دوباره از پنل ادمین شروع کن.")
            return ConversationHandler.END
        ...
        try:
            await context.bot.send_message(uid,
                "💰 موجودی کیف پول شما " + action + " شد.\n"
                "موجودی جدید: " + fmt(new_wallet) + " تومان")
        except: pass
        await update.message.reply_text(
            "✅ " + fmt(abs(amount)) + " تومان " + action + " شد.\n"
            "موجودی جدید کاربر " + str(uid) + ": " + fmt(new_wallet) + " تومان")
...
async def admin_topup_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    parts = update.message.text.split("_")
    # format: /topup_approve_OID_UID_AMOUNT
    # split('_'): ['', 'topup', 'approve', 'OID', 'UID', 'AMOUNT']
    oid, uid, amount = int(parts[3]), int(parts[4]), int(parts[5])
    con = sqlite3.connect("vpn_bot.db")
    cur = con.cursor()
    cur.execute("UPDATE orders SET status='approved' WHERE id=?", (oid,))
    con.commit()
